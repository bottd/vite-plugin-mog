...
pub enum UrlKind<'a> {
    /// `docs/readme.mg`, `/absolute`, `#section`.
    SiteRelative,
...
impl<'a> UrlKind<'a> {
    ...
    /// The only kind path rewriting (`.mg` → `.html`, a `./` prefix) may
    /// touch. `mailto:me@example.mg` ends in `.mg` but is an address, and
...
mod tests {
    ...
    fn url_kind_classifies_by_scheme() {
        ...
        assert_eq!(UrlKind::of("mailto:me@example.mg"), Scheme("mailto"));
        assert_eq!(UrlKind::of("ftp://host/file.mg"), Scheme("ftp"));
        assert_eq!(UrlKind::of("httpserver.mg"), SiteRelative);
        assert_eq!(UrlKind::of("https.mg"), SiteRelative);
    ...
    fn only_web_urls_are_external() {
        ...
        assert!(!UrlKind::of("docs/readme.mg").is_external());
